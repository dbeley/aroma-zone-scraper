...
def get_specs(url):
    ...
    product['Nom'] = str(soup_product.find('div', {'class': 'title-box'}).find('h1').text.strip())
    product['Lien'] = url
    product['Prix'] = ''.join(soup_product.find('span', {'class': 'price'}).find(text=True).split())
    product['Description'] = str(soup_product.find('div', {'class': 'fiche-desc'}).text.strip())
